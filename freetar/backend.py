...
DEFAULT_COLLECTION_LIBRARY = [
    {
        "group": "New group",
        "rows": [{"chords": [{"name": "Am", "shape": "x02210"}]}],
    }
]
DEFAULT_COLLECTION_GROUP_NAME = "My Collection Group"
DEFAULT_COLLECTION_NAME = "My Collection"


def _sanitize_collection_id(collection_id: str | None) -> str:
    return re.sub(r"[^A-Za-z0-9_-]+", "", str(collection_id or ""))


def _resolve_collection_id(collection_id: str | None = None) -> str | None:
    ...
    for grp in groups:
        ...
        for coll in grp.get("collections", []):
            if not isinstance(coll, dict):
                continue
            cid = _sanitize_collection_id(coll.get("id"))
            if cid:
                return cid
    return None
...
def chord_lib_path_for(collection_id: str | None) -> Path:
    """
    Resolve the JSON path for a collection chord library.
    """
    safe_id = _resolve_collection_id(collection_id)
    if not safe_id:
        safe_id = "default"
...
def load_chord_library(collection_id: str | None = None):
    ...
    resolved_id = _resolve_collection_id(collection_id)
    if not resolved_id:
        return []
    path = chord_lib_path_for(resolved_id)
    if not path.exists():
        return []
    try:
...
def save_chord_library(data, collection_id: str | None = None):
    ...
    if not resolved_id:
        return
    try:
        chord_lib_path_for(resolved_id).write_text(
...
def _bootstrap_default_collections(existing_groups: list | None = None):
    groups_list = [grp for grp in (existing_groups or []) if isinstance(grp, dict)]
    existing_ids = {
        _sanitize_collection_id(coll.get("id"))
        for grp in groups_list
        for coll in grp.get("collections", [])
        if isinstance(coll, dict) and _sanitize_collection_id(coll.get("id"))
    }
    default_id = generate_collection_id(existing_ids)
    default_group = {
        "group": DEFAULT_COLLECTION_GROUP_NAME,
        "collections": [{"id": default_id, "name": DEFAULT_COLLECTION_NAME}],
    }
    groups_with_default = [default_group] + groups_list
    save_collections(groups_with_default)
    save_chord_library(DEFAULT_COLLECTION_LIBRARY, default_id)
    return groups_with_default
...
def load_collections():
    ...
    if not COLLECTIONS_PATH.exists():
        return _bootstrap_default_collections()
    try:
        text = COLLECTIONS_PATH.read_text(encoding="utf-8")
        data = json.loads(text)
        if isinstance(data, list):
            has_collections = any(
                isinstance(coll, dict) and _sanitize_collection_id(coll.get("id"))
                for grp in data
                if isinstance(grp, dict)
                for coll in grp.get("collections", [])
            )
            if data and has_collections:
                return data
            return _bootstrap_default_collections(data)
...
def build_chord_view_groups(groups):
    ...
    Input group:
      { "group": "...", "rows": [ { "chords": [ { "name": "...", "shape": "x02210" }, ... ] }, ... ] }

    Output group:
      same, but rows are preserved and each chord also has chord["diagram"] with header/rows.
    """
    result = []
    for g in groups:
        ...
        for row in rows:
            ...
            out_rows.append({"chords": out_chords})
        if not out_rows:
            out_rows.append({"chords": []})
        result.append({"group": group_name, "rows": out_rows})
    return result
...
def build_chord_library_export_payload(groups: list[dict]) -> dict:
    ...
    for grp in groups or []:
        ...
        for row in grp.get("rows", []):
            ...
            for chord in row.get("chords", []):
                if not isinstance(chord, dict):
                    continue
                chords_payload.append(dict(chord))
            rows_payload.append({"chords": chords_payload})
        export_groups.append({"group": group_name, "rows": rows_payload})
    return {"version": 1, "groups": export_groups}
...
def import_settings():
    ...
    if "collections" in payload:
        ...
        if not isinstance(collections_payload, dict):
            return {
                "error": "collections must be an object with groups and libraries"
            }, 400
...
@app.route("/my-chords")
def my_chords():
    collection_id = _resolve_collection_id(request.args.get("collection_id"))
    if collection_id:
        return redirect(url_for("collection_chords", collection_id=collection_id))
    return redirect(url_for("my_collections"))


def _active_collection_id():
    return _resolve_collection_id(request.args.get("collection_id"))
...
def my_chords_import():
    ...
    seen_names = {
        grp.get("group", "") for grp in existing_groups if isinstance(grp, dict)
    }
    ...
    for grp in groups_payload:
        ...
        for row in grp.get("rows", []):
            ...
            rows_payload.append({"chords": chords_payload})
        if not rows_payload:
            rows_payload.append({"chords": []})
        new_groups.append({"group": final_name, "rows": rows_payload})
...
def main(host=None, port=None):
    # Prefer safe, explicit defaults on Windows
    host = host or os.environ.get("FREETAR_HOST", "127.0.0.1")
    port = int(port or os.environ.get("FREETAR_PORT", "22000"))
...
if __name__ == "__main__":
    # Dev mode stays loopback-only unless overridden
    host = os.environ.get("FREETAR_HOST", "127.0.0.1")
    port = int(os.environ.get("FREETAR_PORT", "22000"))
    debug = os.environ.get("FREETAR_DEBUG", "1") == "1"
